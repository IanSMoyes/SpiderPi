...
def serial_servo_write_cmd(pi, id, w_cmd, dat1=None, dat2=None):
    ...
    buf = bytearray(b'\x55\x55')  # 帧头 Define the data frame header
    ...
    UART.write(buf)  # 发送 Transmit data frame over UART
...
def collect_data(r_cmd):
    ...
    if count != 0: # 如果接收到的数据不空 If the buffer is not empty
        ...
        try:
            data_ok = recv_data[-1] == checksum(recv_data[:-1]) # Do the checksums match?
            data_ok = data_ok and recv_data[4] == r_cmd # Do the receive commands match?
            # Does the data have a valid header
            data_ok = data_ok and recv_data[0] == 0x55 and recv_data[1] == 0x55
            if data_ok:
