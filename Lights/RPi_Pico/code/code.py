import board  # CircuitPython library to map functions to GPIO pins
import busio  # CircuitPython library for input/output buses
import neopixel # Guess we'd better design our own then
import time # Standard library for date & time functions

print("Waiting a minute while the Raspberry Pi gets it's head straight")
# time.sleep(60)
print("OK, ready? Let's twinkle!")

UART = busio.UART(tx=board.GP0, rx=board.GP1, baudrate=115200, timeout=1.0)  # Define UART port
...
HEADTAILPIXELS = 16
headtailpixels = neopixel.NeoPixel(board.GP2, HEADTAILPIXELS, brightness=0.4, pixel_order=neopixel.GRBW, auto_write=False)
# NeoPixel ring of 12 pixels on D0 with autowrite on
# These will become the face lights
FACEPIXELS = 12
facepixels = neopixel.NeoPixel(board.GP3, FACEPIXELS, brightness=0.05, pixel_order=neopixel.GRBW, auto_write=True)
...
def read_data():
    ...
    if recv_data is not None:  # If the buffer was not empty
...
def left():  # This is what to do if the robot is turning left
    ...
    if flashon:  # Indicate left
        ...
        headtailpixels[15] = (AMBER)  # In USA you'll want the rear indicators RED
    else:
...
def hazards():  # this is what to do if there is a hazard
    ...
    if flashon:  # Flash all the indicators
        headtailpixels[0] = (AMBER)
        headtailpixels[7] = (AMBER)
        headtailpixels[8] = (AMBER)  # In USA you'll want the back indicators RED
...
def stopped():  # this is what to do if the robot is stopped
    for i in range(HEADTAILPIXELS):
        headtailpixels[i] = (BLACK)  # Turn off all the lights
...
def forward():  # This is what to do if the robot is going forwards
    ...
    for i in range(2,6):
        headtailpixels[i] = (BLACK)  # Turn off the reversing lights
    for i in range(9, 15):
        headtailpixels[i] = (WHITE)  # Turn on the head lights
...
def back():  # This is what to do if the robot is reversing
    ...
    if flashon:  # Flash the reversing lights
        for i in range(1, 7):
            headtailpixels[i] = (GREY)
    else:
        for i in range(1, 7):
            headtailpixels[i] = (BLACK)
    for i in range(9, 15):
        headtailpixels[i] = (BLACK)  # Turn the headlights out
...
direction = 0
# Used to store the rotational state of the robot
# (left, right, straight or hazard)
turning = 0
...
while True:  # forever
    ...
    if current >= starttime + 1:  # Switch state of indicators
        ...
        if direction == 2:
            back()  # Flash the appropriate lights
        if turning == 1:
            right()
        elif turning == 2:
            left()
    ...
    if LED is None:  # If there's no data turn the face lights out
        ...
        if LED == 12:  # If the data is a direction code
            ...
            if direction == 0:
                stopped()  # Call the cooresponding direction function
            elif direction == 1:
                forward()
            if turning == 0:
                straight()  # Call the correspondng turning function
