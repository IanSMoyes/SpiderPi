...
TRINKET_FRAME_HEADER = 0x25 # Data frame header

# Set up UART port
UART = serial.Serial('/dev/ttyAMA0', 115200, timeout = 1.0, rtscts = 0)

def checksum(buf, header=0x25):
    ...
    sum = sum - header - header  # Remove the frame header, the 2 0x25 or 0x55
...
def TrinketM0_write_data(id, colour):
    ...
    buf = bytearray(b'\x25\x25')  # Define the data frame header
    ...
    print(buf)
    # UART.write(buf)  # Transmit data frame over UART
